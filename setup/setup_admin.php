...
if ($tables_result) {
    ...
    if (mysqli_num_rows($tables_result) > 0) {
        ...
        echo "<div class='status-box status-info'>No tables found.</div>";

    }

} else {

    echo "<div class='status-box status-error'>Query Error: ".mysqli_error($koneksi)."</div>";
...
$roleExists = ($roleColumnCheck && mysqli_num_rows($roleColumnCheck) > 0);
if ($roleExists) {
    $selectFields .= ", role";
}

$users = mysqli_query($koneksi, "SELECT $selectFields FROM users");
...
if ($users) {
    ...
    if (mysqli_num_rows($users) > 0) {
        ...
        while ($user = mysqli_fetch_assoc($users)) {
           ...
           echo "<td>" . (isset($user['role']) ? $user['role'] : '') . "</td>";
            echo "</tr>";
        ...
        <a href='setup_admin.php' class='action-btn'>Create Admin</a>
        </div>";
